...
def binaryMask(img):
    ...
    img = cv2.GaussianBlur(img, (7,7), 3) #blur image
    img = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 2) #make it white and black with boundries
    ret, new = cv2.threshold(img, 25, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU) #swap white with black
    return new
